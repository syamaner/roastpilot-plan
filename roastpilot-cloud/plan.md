...
**Superseded 15 Sep 2026: the autonomous CI factory was decommissioned
(permanent, non-compliant on ToS grounds). C2 onward builds through the
compliant interactive model instead: interactive Claude Code as PM /
orchestrator, Codex-MCP as the default implementer, Claude review sub-agents,
and a human merge. [`factory.md`](factory.md) is retained as a historical design
archive; the cloud repo's `docs/state/registry.md` is the authoritative
decommission record.**
...
| D98 | Build process | **Factory-first for C2–C8** _(superseded 15 Sep 2026)_ | Issue-driven agent pipeline (triage → implement → review) on GitHub Actions + `claude-code-action`; the human specs, clarifies, and merges. C1 and the factory itself (new epic F1) are built conventionally. Full spec, security model, label taxonomy, and autonomy ratchet: [`factory.md`](factory.md). Merging is never autonomous; the agent repo is explicitly out of factory scope. **Superseded 15 Sep 2026: the autonomous CI factory was decommissioned permanently on ToS grounds. C2 onward builds through the compliant interactive model (interactive Claude Code orchestrator + Codex-MCP implementer + Claude review sub-agents + human merge); some early C2 stories had shipped factory-first before the pause. `factory.md` is now a historical design archive; the cloud `docs/state/registry.md` carries the decommission record.** |
...
| Delete (was `DELETE`) | `CALL delete_roast(id)`: procedural cascade (reviews, telemetry, artifact rows, `REMOVE @stage` files — by a directory prefix the procedure constructs from the roast's validated `idempotency_key`, `@roast_artifacts/<run_id>/`, not by iterating the recorded `roast_artifacts.stage_path` values and not under a `<roast_id>/` prefix; see D-341-A) + summary recompute. (C2-S6/#314 ships the row-cascade first; the `REMOVE @stage` step lands with the C3 connector's `stage_path` contract — #341, D-314-I — gated before C3 writes artifacts.) |
