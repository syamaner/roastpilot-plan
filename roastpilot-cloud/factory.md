...
## D153: D147 gate-2's thumbs-up surface corrected for the human Merge Policy — the connector SWAPS 👀→👍, so the anti-spoof binds on the 👍's own bot-authorship, not an observable preceding eyes reaction

**Decision (operator, AskUserQuestion, 18 Aug 2026), refining D147 gate-2.** D147 gate-2 accepted a bot thumbs-up as a clean surface "valid only when **both the preceding eyes reaction and the thumbs-up are bot-authored** ... so that on a public repo no stranger can supply half the verdict." Direct live observation establishes that premise is factually stale: the Codex connector **withdraws its own eyes reaction at the instant it posts the thumbs-up** (roastpilot-cloud PR #304, 18 Aug 2026: bot eyes at 20:07:57Z, bot `+1` at 20:10:00Z, the eyes gone; consistent with the `codex-connector-clean-signal-calibration` record and #235's reaction-volatility finding). The eyes and thumbs do not coexist as an observable pair in the common case, so requiring an observable preceding bot eyes reaction as a precondition on the thumbs is unsatisfiable, and a watcher applying it literally will wrongly reject a genuinely clean verdict.

**Refinement (human Merge Policy path only).** For the human Merge Policy acceptance in `AGENTS.md`, a bot-authored thumbs-up whose OWN author is `chatgpt-codex-connector[bot]` (verified through the reactions-API `user.login`) is a valid clean surface **without** requiring an observable preceding eyes reaction. The anti-spoof property D147 gate-2 protects is preserved, relocated from "an observable preceding bot eyes" to "bot-authorship of the thumbs-up itself", which a public-repo stranger cannot forge, so no third party can supply half the verdict. The distinct requirement that an in-progress eyes reaction, when relied on as the look-first / keep-waiting signal, must itself be bot-authored is **retained unchanged** (a stranger's eyes must never make the operator wait out the timeout for a review that never started).
