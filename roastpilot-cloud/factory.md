...
- Success is admitted ONLY on the four-conjunct signature `metadata_only==true && result_clean==true && result_num_turns=="1" && tool_invocations=="0"`, all derived from the **trusted SDK execution transcript** (base-action-authored, not model-authored). Any missing / malformed / ambiguous field fails **closed** (falls through to the existing checklist/sentinel grammar).
...
- The two new envelope fields (`result_num_turns`, `tool_invocations`) are double-gated to canonical integers (jq validity + `^(0|[1-9][0-9]*)$`) before write — no `$GITHUB_OUTPUT` injection, no model-derived bytes.
