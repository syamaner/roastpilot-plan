...
| D150 | Decision-number convention across the plan repo | **ONE shared D-number series for the whole `roastpilot-plan` repo; next free is one past the highest number defined ANYWHERE in it.** D103-D149 carry a permanent collision and MUST be cited with their file. No existing number is renumbered. | Restores the convention the repo actually used up to D102: D1-D4 are cross-repo, `roastpilot-agent/plan.md` never defines D9-D14 or D97-D101 (D10-D13 and D97-D101 live in `roastpilot-cloud/plan.md`, D9 in `roastpilot-agent/README.md`, D14 in this file), and D2 is superseded by D97 from a different file — D5-D99 interleave with zero collisions. From D103 both `roastpilot-agent/plan.md` and `roastpilot-cloud/factory.md` began incrementing independently, so **both define D103-D129** for unrelated decisions (`plan.md` D108 is the PR-scoped Claude approval bridge `AGENTS.md` cites; `factory.md` D108 is a cloud slice decision; D122/D124/D127 collide likewise and are all cited by RP-B). Renumbering was rejected: D-numbers appear in merged commit messages, closed issues, merged PR bodies and `AGENTS.md`, which cannot be rewritten, so a renumber would break immutable references to fix a historical ambiguity that citation discipline already handles. Per-programme prefixes were rejected as a second convention to hold alongside the bare historical numbers. **Deriving the next number is a known trap** — three attempts got it wrong on 9 Aug 2026 alone. Decisions are defined in FIVE formats and no single grep finds them all: `\| D<n> \|` table rows (`00-repository-structure.md`, `roastpilot-agent/plan.md`, `roastpilot-cloud/plan.md`), `**D<n> (date) — ...**` paragraphs (`factory.md` to D143), `## D<n>: ...` headings (`factory.md` from D145), `**D<n> — ...**` bold paragraphs carrying the date inside the sentence (this file, where D14 lives), and `- **D<n>** (date): ...` bullets (`roastpilot-agent/README.md`, where D9 lives). D144 is the only real gap, deliberately renumbered to D145: not a free number. **Correction (9 Aug 2026, roastpilot-agent#750):** as first written this rationale listed only three formats and asserted that neither D9 nor D14 is defined anywhere. Both are defined — in exactly the two formats the three-format inventory omitted, which is why the audit missed them. Re-derived across all five formats the highest number defined anywhere is still D150, so the convention and the next free number are unchanged; only this rationale's supporting claim was wrong. (9 Aug 2026, operator-ratified; surfaced by roastpilot-agent#734.) |
